...
# Uvicorn access log'u sustur (dashboard 5s polling logları kirletmesin)
...
_last_trade_ts: float = 0.0          # legacy — artık kullanılmıyor, silinmedi (uyumluluk)
_last_trade_ts_per_symbol: dict[str, float] = {}   # per-symbol cooldown
...
def init_services() -> None:
    ...
    _eth_price = None
    try:
        _btc_price = float(binance_svc.get_ticker_price("BTC/USDT") or 0)
        _eth_price = float(binance_svc.get_ticker_price("ETH/USDT") or 0)
        logger.info("grid_init_prices BTC=%.2f ETH=%.2f", _btc_price, _eth_price)
    except Exception as _e:
        logger.warning("grid_init_price_fetch failed: %s", _e)
    
    strategy = TradingStrategy(
        symbol="BTC/USDT",
        initial_price=_btc_price if _btc_price and _btc_price > 0 else None
    )
    # ETH grid'ini de başlat
    if _eth_price and _eth_price > 0:
        strategy._init_symbol_grid("ETH/USDT", initial_price=_eth_price)
        logger.info("✅ Grid initialized: BTC=%.2f ETH=%.2f", _btc_price or 0, _eth_price)
    _snapback_enabled = os.getenv("SNAPBACK_ENABLED", "true").lower() == "true"
    if _snapback_enabled:
        snapback_strategy = SnapbackStrategy()
        logger.info("✅ Snapback strategy aktif.")
    else:
        snapback_strategy = None
        logger.warning("⚠️ Snapback strategy devre dışı (SNAPBACK_ENABLED=false)")

    # ── GRID STATE RESTORE (restart-safe) ─────────────────────────────
    # DB'de kayıtlı grid seviyeleri varsa geri yükle.
    # Bu sayede restart sonrası açık grid pozisyonları korunur.
    _grid_symbols = ["BTC/USDT", "ETH/USDT"]
    for _gsym in _grid_symbols:
        try:
            _saved = db.load_grid_state(_gsym)
            if _saved:
                ok = strategy.restore_grid_state_from_dict(_gsym, _saved)
                if ok:
                    logger.info("✅ Grid state restored from DB: %s (updated_at=%s)", _gsym, _saved.get("_db_updated_at"))
                else:
                    logger.warning("⚠️ Grid state restore failed for %s — starting fresh", _gsym)
            else:
                logger.info("grid_state_not_found symbol=%s — fresh start", _gsym)
        except Exception as _ge:
            logger.warning("grid_state_restore_error symbol=%s err=%s", _gsym, str(_ge)[:200])
    # ── GRID STATE RESTORE END ────────────────────────────────────────
        ...
        try:
            if db:
                ...
                if tick % 12 == 0:
                    ...
                    # Exit Engine — yaşlı/zararlı pozisyonları kapat
                    if exit_engine and db:
                        try:
                            _positions = db.get_open_positions()
                            # Grid state'teki envanteri de ekle (BTC/ETH gibi positions
                            # tablosuna yazılmayan ama grid_state'te yaşayan pozisyonlar)
                            if strategy:
                                _pos_syms = {normalize_symbol(p.get("symbol") or "") for p in _positions}
                                for _gsym in (strategy._symbol_grids if hasattr(strategy, "_symbol_grids") else {}).keys():
                                    _gnorm = normalize_symbol(_gsym)
                                    if _gnorm in _pos_syms:
                                        continue
                                    try:
                                        _gs = strategy._get_symbol_grid(_gsym)
                                        if _gs is None:
                                            continue
                                        # Her inventory dolu seviye için pseudo-pozisyon oluştur
                                        for _gl in (_gs.grids if hasattr(_gs, "grids") else []):
                                            if getattr(_gl, "quantity", 0) > 1e-10:
                                                _opened = getattr(_gl, "bought_at_ms", None)
                                                _opened_str = None
                                                if _opened:
                                                    import datetime
                                                    _opened_str = datetime.datetime.fromtimestamp(
                                                        _opened / 1000
                                                    ).isoformat()
                                                _positions.append({
                                                    "symbol": _gsym,
                                                    "amount": float(_gl.quantity),
                                                    "entry_price": float(_gl.price),
                                                    "cost": float(_gl.price * _gl.quantity),
                                                    "strategy_name": "grid_v1",
                                                    "opened_at": _opened_str or "2026-03-19T04:00:00",
                                                    "highest_price": None,
                                                    "stop_loss": None,
                                                    "take_profit": None,
                                                    "grid_inventory": True,
                                        ...
                                        logger.debug("exit_engine grid_state_pos_build err sym=%s: %s", _gsym, _gse)
                            # Price cache'de eksik semboller için fiyat çek
                            _ee_price_cache = dict(_WATCHDOG_TICK_PRICE_CACHE)
                            for _p in _positions:
                                _s = normalize_symbol(_p.get("symbol") or "")
                                if _s and _s not in _ee_price_cache:
                                    try:
                                        _px = _safe_price(_s)
                                        if _px and _px > 0:
                                            _ee_price_cache[_s] = _px
                                    except Exception:
                                        pass
                            _closed = exit_engine.run(_positions, _ee_price_cache)
                            if _closed:
                                logger.info("exit_engine_closed count=%s", _closed)
                                # EXIT_STOP_LOSS sonrası re_entry_guard: cooldown başlat
                                for _cl in (db.get_open_positions() if hasattr(_closed, '__iter__') else []):
                                    pass
                                # exit_engine kapatılan pozisyonlar için guard
                                try:
                                    _after_positions = {p.get("symbol") for p in db.get_open_positions()}
                                    for _pos in _positions:
                                        _sym = _pos.get("symbol")
                                        if _sym and _sym not in _after_positions:
                                            _entry = float(_pos.get("entry_price") or 0)
                                            _px = float(_WATCHDOG_TICK_PRICE_CACHE.get(_sym) or _entry or 0)
                                            if _px > 0:
                                                ...
                                                logger.info(
                                                    "exit_engine_re_entry_guard sym=%s cooldown başlatıldı",
                                                    _sym,
                                                )
                                except Exception:
                                    pass
                        except Exception as _ee:
                            logger.warning("exit_engine_run_error: %s", _ee)

                    # Grid inventory zorla kapatıldıysa grid_state'i temizle
                    # (BTC/ETH gibi positions tablosunda olmayan pozisyonlar)
                    if strategy and db:
                        try:
                            for _gsym in list(
                                strategy._symbol_grids.keys()
                                if hasattr(strategy, "_symbol_grids") else []
                            ):
                                _gs = strategy._get_symbol_grid(_gsym)
                                if _gs is None:
                                    continue
                                ...
                                for _gl in (_gs.grids if hasattr(_gs, "grids") else []):
                                    if getattr(_gl, "quantity", 0) > 1e-10:
                                        # Bu sembol positions tablosunda var mı?
                                        _in_positions = any(
                                            normalize_symbol(p.get("symbol") or "") == normalize_symbol(_gsym)
                                            for p in (db.get_open_positions() or [])
                                        )
                                        if not _in_positions:
                                            # Grid state'te var ama positions'da yok
                                            # AGE exit başarısız olmuştur → manuel temizle
                                            _px = _WATCHDOG_TICK_PRICE_CACHE.get(normalize_symbol(_gsym)) or _safe_price(normalize_symbol(_gsym))
                                            if _px and _px > 0:
                                                _pnl = (_px - _gl.price) / _gl.price if _gl.price > 0 else 0
                                                logger.warning(
                                                    "grid_inventory_force_clear sym=%s level=%.4f qty=%.6f pnl=%.2f%%",
                                                    _gsym, _gl.price, _gl.quantity, _pnl * 100
                                                )
                                                _gl.quantity = 0.0
                                                _changed = True
                                if _changed:
                                    db.save_grid_state(_gsym, strategy.grid_state_to_dict(_gsym))
                        except Exception as _gce:
                            logger.debug("grid_inventory_clear_error: %s", _gce)
...
async def scanner_loop() -> None:
    ...
    round_no = 0
    _last_daily_summary_day: str | None = None
    ...
    while True:
        ...
        try:
            ...
            now = time.time()
            # Per-symbol cooldown: her coin kendi sayacını tutar.
            # Sadece TÜM coinler cooldown'daysa bekle.
            if COOLDOWN_SECONDS > 0:
                active_coins_preview = get_universe_symbols()
                if active_coins_preview:
                    all_cooling = all(
                        (now - _last_trade_ts_per_symbol.get(normalize_symbol(s), 0.0)) < COOLDOWN_SECONDS
                        for s in active_coins_preview
                    )
                    if all_cooling:
                        min_remaining = min(
                            int(COOLDOWN_SECONDS - (now - _last_trade_ts_per_symbol.get(normalize_symbol(s), 0.0)))
                            for s in active_coins_preview
                        )
                        logger.info("⏳ Tüm coinler cooldown'da (%ss kaldı). 15s bekleniyor...", min_remaining)
            ...
            for symbol in active_coins:
                try:
                    ...
                    # Grid rebalance bildirimi
                    if reason == "grid_rebalanced":
                        _new_range = details.get("new_range", "?")
                        _rebal_price = float(details.get("current_price") or 0.0)
                        logger.info("grid_rebalanced sym=%s new_range=%s", symbol, _new_range)
                        if notifier and getattr(notifier, "enabled", False):
                            notifier.notify_grid_event(
                                symbol=symbol, event="GRID_REBALANCE", price=_rebal_price,
                            )
                    ...
                    if decision in ("BUY", "SELL"):
                        if trading_allowed and not execution_blocked:
                            ...
                            if (now - last_ts) >= _BLOCKED_REPORT_TTL_S:
                                ...
                                logger.debug("📢 RAPOR(blocked): %s", res)
            ...
            # Günlük özet: her gün ilk scanround'da gönder (TR saatiyle)
            _today_tr = datetime.now(tz=TR_TZ).date().isoformat()
            if (
                _last_daily_summary_day != _today_tr
                and notifier and getattr(notifier, "enabled", False)
                and db
            ):
                _last_daily_summary_day = _today_tr
                try:
                    _daily_pnl  = float(db.get_today_realized_pnl())
                    _open_cnt   = len(db.get_open_positions())
                    _summary    = db.get_dashboard_summary()
                    _trade_cnt  = int(_summary.get("trade_count", 0))
                    _equity     = _paper_equity_usdt() if IS_TEST_MODE else 0.0
                    _wr         = _summary.get("win_rate")
                    _try_create_task(
                        notifier.send_daily_summary(
                            realized_pnl=_daily_pnl,
                            open_positions=_open_cnt,
                            total_trades=_trade_cnt,
                            equity=_equity,
                            win_rate=float(_wr) if _wr is not None else None,
                        )
                    )
                except Exception as _ds_err:
                    logger.warning("daily_summary_failed: %s", _ds_err)
...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        logger.info("universe_loaded symbols=%s updated_at=%s (DB snapshot, V1_FORCE_UNIVERSE override applies below)", u.get("symbols"), u.get("updated_at"))
    except Exception as e:
    ...
    # Exit Engine başlat
    global exit_engine
    if trader_bot and db:
        exit_engine = ExitEngine(db=db, trader=trader_bot)
        logger.info("Exit Engine v0.1 aktif.")
    ...
    if notifier and getattr(notifier, "enabled", False):
        try:
            _syms = get_universe_symbols()
            _btc_gs = strategy._symbol_grids.get("BTC/USDT") if strategy else None
            _paper_bal = float(db.get_paper_balance("USDT")) if db and IS_TEST_MODE else None
            await notifier.send_startup_alert(
                mode="test" if IS_TEST_MODE else "live",
                symbols=_syms,
                grid_lower=_btc_gs.grid_lower if _btc_gs else None,
                grid_upper=_btc_gs.grid_upper if _btc_gs else None,
                paper_balance=_paper_bal,
            )
        except Exception as _se:
            logger.warning("startup_alert_failed: %s", _se)
...
def risk_watchdog_manage_positions() -> None:
    ...
    Strateji bazlı davranış:
      - grid_v1  : Watchdog SL/TP/trailing UYGULAMAZ.
                   Grid'in kendi crossing mantığı pozisyonu yönetir.
                   Sadece acil durum (emergency_stop) eşiğini izler.
      - snapback : Kısa vadeli strateji; sabit SL/TP, trailing yok.
      - diğerleri: Standart trailing stop + SL/TP.

    Fail-closed: fiyat alınamazsa hiçbir şey yapma.
    """
    ...
    # Grid acil durum eşiği: grid aralığının bu kadar dışına çıkarsa kapat
    grid_emergency_stop_pct = float(os.getenv("GRID_EMERGENCY_STOP_PCT", "0.05"))

    for p in positions:
        try:
            sym          = normalize_symbol(p.get("symbol"))
            amt          = float(p.get("amount", 0.0) or 0.0)
            strategy_name = str(p.get("strategy_name") or "").lower().strip()
            ...
            # ── GRID POZİSYONLARI ──────────────────────────────────────
            # Grid kendi crossing mantığıyla çalışır. Watchdog üç şeyi izler:
            #   1. Acil durum: fiyat grid aralığının çok dışına çıktı → kapat
            #   2. TREND rejimi: yeni BUY zaten scanner'da bloklu; ama mevcut
            #      açık pozisyon girdaba düşerse → soft SL uygula (entry * %loss_pct)
            #   3. HIGH_VOL: aşırı volatilitede grid tüm pozisyonları kapatır
            if strategy_name == "grid_v1":
                # Grid aralığını strategy cache'inden al
                try:
                    grid_lower = entry * 0.85
                    grid_upper = entry * 1.15
                    if strategy:
                        gs = strategy._symbol_grids.get(sym)
                        if gs:
                            grid_lower = gs.grid_lower
                            grid_upper = gs.grid_upper
                except Exception:
                    pass

                range_size = grid_upper - grid_lower
                emergency_threshold = range_size * grid_emergency_stop_pct

                # Acil durum: fiyat grid aralığının dışına çok çıktı
                below_emergency = px < (grid_lower - emergency_threshold)
                above_emergency = px > (grid_upper + emergency_threshold)

                if below_emergency or above_emergency:
                    direction = "below" if below_emergency else "above"
                    logger.warning(
                        "watchdog_grid_emergency sym=%s px=%.2f grid=[%.2f,%.2f] direction=%s",
                        sym, px, grid_lower, grid_upper, direction,
                    )
                    trader_bot.execute_trade(sym, "SELL", px, 1.0, reason="GRID_EMERGENCY_EXIT")
                    if notifier and getattr(notifier, "enabled", False):
                        notifier.notify_grid_event(
                            symbol=sym, event="GRID_EMERGENCY_EXIT", price=px,
                            pnl=(px - entry) * amt if entry > 0 else None,
                        )
                    continue

                # Regime bazlı soft koruması
                # TREND veya HIGH_VOL'da pozisyon zararda ve ağır kayıp eşiğini geçtiyse kapat.
                # Backtest'te emergency_exit'lerden kaynaklanan büyük kayıpları önler.
                try:
                    cached_regime = strategy.get_cached_regime(sym) if strategy else None
                    if cached_regime:
                        regime_now = str(cached_regime.get("regime", "UNKNOWN"))
                        grid_trend_loss_pct = float(os.getenv("GRID_TREND_LOSS_CLOSE_PCT", "0.04"))
                        pnl_pct = (px - entry) / entry if entry > 0 else 0.0

                        if regime_now in ("TREND", "HIGH_VOL") and pnl_pct <= -grid_trend_loss_pct:
                            logger.warning(
                                "watchdog_grid_trend_close sym=%s regime=%s px=%.2f entry=%.2f "
                                "pnl=%.2f%% threshold=%.2f%%",
                                sym, regime_now, px, entry,
                                pnl_pct * 100, grid_trend_loss_pct * 100,
                            )
                            trader_bot.execute_trade(
                                sym, "SELL", px, 1.0, reason=f"GRID_TREND_CLOSE({regime_now})"
                            )
                            # Trend close sonrası re-entry guard: loss ile kapandı,
                            # cooldown süresince yeni BUY açılmasın
                            try:
                                re_entry_guard.mark_sell(
                                    symbol=sym,
                                    sell_price=px,
                                    entry_price=entry,
                                    reason=f"GRID_TREND_CLOSE({regime_now})"
                                )
                            except Exception:
                                pass
                            if notifier and getattr(notifier, "enabled", False):
                                notifier.notify_grid_event(
                                    symbol=sym,
                                    event="GRID_TREND_CLOSE",
                                    price=px,
                                    pnl=pnl_pct * entry * amt if entry > 0 else None,
                                    regime=regime_now,
                                )
                            continue
                except Exception as _re:
                    logger.debug("watchdog_regime_check_failed sym=%s err=%s", sym, _re)

                # Grid için başka müdahale yok — crossing scanner halleder
                continue

            # ── SNAPBACK / DİĞER POZİSYONLAR ──────────────────────────
            sl = p.get("stop_loss")
            ...
            # Snapback: sabit SL/TP, trailing yok (kısa vadeli strateji)
            is_snapback = strategy_name == "snapback"

            if not is_snapback:
                # Trailing stop sadece directional long stratejilerde
                if px > hp:
                    hp = px
                    try:
            ...
            if px <= float(sl):
                logger.info("watchdog_stop_loss sym=%s px=%.4f sl=%.4f strategy=%s", sym, px, sl, strategy_name)
                trader_bot.execute_trade(sym, "SELL", px, 1.0, reason="STOP_LOSS")
                # Stop-loss sonrası re-entry guard: cooldown başlat
                try:
                    _entry_px = float(p.get("entry_price") or px)
                    re_entry_guard.mark_sell(
                        symbol=sym,
                        sell_price=px,
            ...
            if px >= float(tp):
                logger.info("watchdog_take_profit sym=%s px=%.4f tp=%.4f strategy=%s", sym, px, tp, strategy_name)
...
def get_positions_endpoint():
    ...
    logger.debug("dashboard_positions db_path=%s row_count=%s", db.db_path, len(open_positions))
    result = []
...
@app.get("/debug/grid_state")
def debug_grid_state():
    """Grid state'i DB'den oku + bellekteki canlı state ile karşılaştır."""
    if not db:
        raise HTTPException(status_code=500, detail="DB hazir degil")

    db_states = db.list_grid_states()

    live_states = []
    if strategy:
        for sym in ["BTC/USDT", "ETH/USDT"]:
            try:
                live_states.append(strategy.grid_state_to_dict(sym))
            except Exception as e:
                live_states.append({"symbol": sym, "error": str(e)})

    return {"db": db_states, "live": live_states}
...
async def run_analysis(symbol: str, timeframe: str, execute_trade: bool = False, risk_multiplier: float = 1.0):
    global _last_trade_ts, _last_trade_ts_per_symbol
    ...
    try:
        ...
        if str(signal_data.get("signal", "HOLD")).upper() == "HOLD" and snapback_strategy is not None:
            # TREND FİLTRESİ: ADX>40 veya ER>0.5 ise snapback BUY engelle
            _snap_adx = float(signal_data.get("adx") or 0.0)
            _snap_er  = float(signal_data.get("er")  or 0.0)
            _snap_trend_block = (_snap_adx > 40.0) or (_snap_er > 0.5)
            if _snap_trend_block:
                logger.warning(
                    "snapback_trend_filter_blocked sym=%s adx=%.1f er=%.3f → snapback engellendi",
                    symbol, _snap_adx, _snap_er,
                )
            else:
                ...
                if snapback_signal == "BUY":
                    # Pozisyon açıksa veya inventory limiti doluysa debug'a düşür
                    _sym_positions = []
                    try:
                        if db:
                            _all_pos = db.get_open_positions()
                            _sym_positions = [p for p in _all_pos if p.get("symbol") == symbol]
                    except Exception:
                        pass
                    _has_position = bool(_sym_positions)
                    if _has_position:
                        logger.info(
                            "fallback_override strategy=%s symbol=%s signal=%s blocked=position_already_open",
                            snapback_strategy_name, symbol, snapback_signal,
                        )
                    else:
                        logger.info(
        ...
        now = time.time()
        # Per-symbol cooldown: bu sembol için son trade zamanına bak
        if execute_trade and COOLDOWN_SECONDS > 0:
            sym_key = normalize_symbol(symbol)
            last_ts_for_sym = _last_trade_ts_per_symbol.get(sym_key, 0.0)
            elapsed = now - last_ts_for_sym
            if elapsed < COOLDOWN_SECONDS:
                blocked_reason = f"cooldown({int(COOLDOWN_SECONDS - elapsed)}s)"
        ...
        if can_execute:
            ...
            if trade_result:
                _now_ts = time.time()
                _last_trade_ts = _now_ts  # legacy uyumluluk
                _last_trade_ts_per_symbol[normalize_symbol(symbol)] = _now_ts  # per-symbol cooldown

                # ── GRID STATE GÜNCELLEME ──────────────────────────────
                try:
                    _strat_name = str(signal_data.get("strategy_name", ""))
                    _crossing = signal_data.get("crossing_metadata") or {}
                    if _strat_name == "grid_v1" and _crossing:
                        strategy._record_grid_execution(
                            symbol=symbol,
                            action=str(_crossing.get("action", decision)),
                            grid_price=float(_crossing.get("grid_price", current_price)),
                            quantity=float(_crossing.get("quantity", 0)),
                            trade_success=True,
                        )
                        logger.info(
                            "✅ GRID STATE UPDATED: %s %s @ %.2f qty=%.8f",
                            symbol,
                            _crossing.get("action"),
                            float(_crossing.get("grid_price", 0)),
                            float(_crossing.get("quantity", 0)),
                        )
                        # ── DB PERSIST: grid state kaydet ─────────────
                        try:
                            if db:
                                _gs_dict = strategy.grid_state_to_dict(symbol)
                                db.save_grid_state(symbol, _gs_dict)
                        except Exception as _pe:
                            logger.warning("grid_state_persist_failed symbol=%s err=%s", symbol, str(_pe)[:200])
                        # ── DB PERSIST SON ────────────────────────────
                except Exception as _ge:
                    logger.error("grid_state_update_failed: %s", _ge)
                # ── GRID STATE GÜNCELLEME SON ──────────────────────────

                if decision == "SELL":
                    try:
                        position = db.get_open_position(symbol) if db else None
                        if position:
                            # Grid stratejisinde entry_price = o grid seviyesinin BUY fiyatı
                            # DB'deki entry_price tüm pozisyonun açılış fiyatı olduğundan
                            # grid SELL için crossing_metadata'daki grid_price kullanılmalı
                            _strategy = signal_data.get("strategy_name", "")
                            _crossing = signal_data.get("crossing_metadata", {})
                            if _strategy == "grid_v1" and _crossing.get("grid_price"):
                                _entry_for_guard = float(_crossing["grid_price"])
                            else:
                                _entry_for_guard = float(position.get("entry_price") or 0)
                            re_entry_guard.mark_sell(
                                symbol=symbol,
                                sell_price=current_price,
                                entry_price=_entry_for_guard,
        ...
        if blocked_reason:
            ...
            if execute_trade and decision in ["BUY", "SELL"]:
                logger.debug(
                    "analysis trade_blocked sym=%s decision=%s reason=%s price=%.2f risk_x=%.2f corr=%.2f",
                    symbol,
                    decision,
                    blocked_reason,
                    float(current_price),
...
if __name__ == "__main__":
    ...
    uvicorn.run("backend.main:app", host="0.0.0.0", port=8000, reload=False, log_level="info", access_log=False)