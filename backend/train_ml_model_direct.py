"""
Direct Binance ML Model Training Script

Fetches data directly from Binance API (no research.db needed)

Usage:
    python -m backend.train_ml_model_direct --start 2025-12-01 --end 2026-02-22
"""
...
class DirectMLTrainer:
    """ML trainer that fetches data directly from Binance"""
    ...
    def _fetch_ohlcv(
        ...
        try:
            ...
            # CCXT format is already [timestamp, open, high, low, close, volume]
            return all_candles
    ...
    def collect_training_data(
        ...
        """Collect training data from Binance"""
        
        if symbols is None:
            symbols = ["BTC/USDT", "ETH/USDT"]
        
        print(f"\n{'='*80}")
        print(f"COLLECTING TRAINING DATA FROM BINANCE")
        print(f"{'='*80}")
        ...
        for symbol in symbols:
            ...
            # Fetch OHLCV
            ohlcv = self._fetch_ohlcv(symbol, start_date, end_date)
            ...
            for i in range(window_size, len(ohlcv) - 50, 10):  # Sample every 10 candles
                ...
                try:
                    ...
                    # Look ahead 50 candles (~12.5 hours)
            ...
            print(f"  ✅ Collected {sample_count} samples from {symbol}" + " " * 30)
...
def main():
    parser = argparse.ArgumentParser(description="Train ML model (direct Binance fetch)")
    ...
    print(f"\n{'='*80}")
    print("ML MODEL TRAINING PIPELINE (DIRECT BINANCE)")
    print(f"{'='*80}")
